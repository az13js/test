...
function getData(array $urls, string $cookie, string $reffer, string $userAgent): array
{
    $loader = new MultipleUrlContextLoader();
    $loader->setUrls($urls);
    $loader->setCookie($cookie);
    $loader->setReffer($reffer);
    $loader->setUserAgent($userAgent);
    $loader->loadContent();
    return $loader->getContents();
}

function getText($content): array
{
    $result = [];
    preg_match_all('#title="[\S|\s]*?"#i', $content, $result);
    $results = [];
    foreach ($result as $items) {
        foreach ($items as $item) {
            $row = rtrim(ltrim($item, 'title="'), '"');
            if ('' === $row) {
                continue;
            }
            $results[] = $row;
...
$reffer = 'http://tieba.baidu.com';
$pageTotal = 100;

$urls = [];
for ($i = 0; $i < $pageTotal; $i++) {
    $urls[] = 'http://tieba.baidu.com/f?kw=' . urlencode($tiebaName) . '&ie=utf-8&pn=' . (50 * $i);
}

foreach (getData($urls, $cookie, $reffer, $userAgent) as $id => $content) {
    if (!$content) {
        continue;
    }
    foreach (getText($content) as $str) {
        file_put_contents($id . '.txt', $str . PHP_EOL, FILE_APPEND);
    }
}